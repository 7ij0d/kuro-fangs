 ...
 * Videos, Audio Recordings, Images/Atlas, Profile, Alerts & Search
 */
...
const SecondaryPages = {
  ...
  renderAudioRecordings(container, queryParams) {
    ...
    const targetSubject = queryParams?.get('subject') || 'all';
    let currentFilter = targetSubject;
    let currentView = localStorage.getItem('kf_recordings_view') || 'grid'; // 'grid' | 'list'
    let searchQuery = '';

    const baseRecordings = window.DATA?.recordings || [];

    const renderList = async () => {
      let targetSheets = currentFilter === 'all'
        ? (window.DATA?.sheets || [])
        : (window.DATA?.sheets || []).filter(s => s.subject_id === currentFilter);

      // Apply search filter
      if (searchQuery) {
        const q = searchQuery.toLowerCase();
        targetSheets = targetSheets.filter(sheet => {
          const title = (sheet.title_ar || sheet.title_en || sheet.title || '').toLowerCase();
          const subjObj = subjects.find(s => s.id === sheet.subject_id);
          const subjName = subjObj ? ((isAr ? subjObj.name_ar : subjObj.name_en) || '').toLowerCase() : '';
          return title.includes(q) || subjName.includes(q);
        });
      }
      ...
      // Update count
      const withRecs = targetSheets.filter(s => baseRecordings.some(r => r.sheet_id === s.id)).length;
      const countEl = document.getElementById('recordings-count-label');
      if (countEl) countEl.textContent = isAr ? `${withRecs} شيت` : `${withRecs} Sheets`;

      if (targetSheets.length === 0) {
        gridEl.className = '';
        gridEl.innerHTML = `
          <div class="empty-state-card" style="padding:44px 24px;text-align:center;">
            <h3 class="empty-state-title">${isAr ? 'لا توجد نتائج' : 'No results found'}</h3>
            <p class="empty-state-subtitle">${isAr ? 'جرّب كلمة بحث أخرى أو غيّر المادة' : 'Try a different search term or subject'}</p>
          </div>
        `;
        if (window.lucide) window.lucide.createIcons();
        return;
      ...
      const itemsHtml = await Promise.all(targetSheets.map(async sheet => {
        ...
        // Build track HTML
        const recsHtml = sheetRecs.length === 0 ? '' : (await Promise.all(sheetRecs.map(async rec => {
          let audioSrc = rec.audio_url || '';
          if (!audioSrc && window.DATA?.audioStore?.getAudioUrl) {
            try { audioSrc = await window.DATA.audioStore.getAudioUrl(rec.id) || ''; } catch (e) {}
          }
            ...
            <div class="rec-track-item">
              <div class="rec-track-info">
                ...
                <span class="rec-track-meta">
                  ${rec.doctor ? `<span class="sgc-meta-item"><i data-lucide="user-round" class="sgc-meta-icon"></i>${isAr ? 'د.' : 'Dr.'} ${rec.doctor}</span>` : ''}
                  ${rec.duration ? `<span class="sgc-meta-sep">·</span><span class="sgc-meta-item"><i data-lucide="clock" class="sgc-meta-icon"></i>${rec.duration}</span>` : ''}
                </span>
              </div>
        ...
        if (currentView === 'list') {
          // LIST ROW
          return `
            <div class="sheets-list-row ${sheetRecs.length === 0 ? 'rec-card-empty' : ''}">
              <div class="slr-left">
                <span class="sgc-subject-badge" style="background:${color.bg};color:${color.text};border-color:${color.border};">${subjName}</span>
              </div>
              <div class="slr-info" style="flex:1;min-width:0;">
                <div class="slr-title">${sheetName}</div>
                <div class="sgc-meta" style="margin-top:4px;">
                  <span class="sgc-meta-item">
                    <i data-lucide="headphones" class="sgc-meta-icon"></i>
                    ${sheetRecs.length} ${isAr ? 'تسجيل' : sheetRecs.length === 1 ? 'Recording' : 'Recordings'}
                  </span>
                </div>
                ${sheetRecs.length > 0 ? `<div class="rec-tracks-list" style="margin-top:8px;">${recsHtml}</div>` : ''}
              </div>
            </div>`;
        }
        ...
        // GRID CARD
        return `
          <div class="sheets-grid-card ${sheetRecs.length === 0 ? 'rec-card-empty' : 'rec-card'}">
            <div class="sgc-header">
              <span class="sgc-subject-badge" style="background:${color.bg};color:${color.text};border-color:${color.border};">${subjName}</span>
              ${sheetRecs.length > 0 ? `
                <span class="sgc-verified-badge">
                  <i data-lucide="headphones" class="sgc-meta-icon" style="color:var(--brand-burgundy);width:12px;height:12px;"></i>
                  ${sheetRecs.length} ${isAr ? 'تسجيل' : sheetRecs.length === 1 ? 'Rec' : 'Recs'}
                </span>` : ''}
            </div>
            <h3 class="sgc-title" ${sheetRecs.length === 0 ? 'style="color:var(--text-muted);"' : ''}>${sheetName}</h3>
            ${sheetRecs.length === 0 ? `
              <div class="sgc-meta"><span class="sgc-meta-item" style="color:var(--text-muted);">
              ...
              <div class="rec-tracks-list">${recsHtml}</div>`}
          </div>`;
      }));

      gridEl.className = currentView === 'grid' ? 'sheets-grid-container' : 'sheets-list-container-view';
    ...
    container.innerHTML = `
      <div class="page-title-bar">
        <div class="page-title-group" style="display:flex;align-items:center;gap:16px;">
          <div style="width:58px;height:58px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle,rgba(200,67,67,0.12) 0%,transparent 70%);">
            <img src="assets/icons/recordings_icon.png" alt="Recordings" style="width:46px;height:46px;border-radius:50%;object-fit:cover;border:2px solid var(--border-subtle);box-shadow:0 2px 8px rgba(0,0,0,0.12);" />
          </div>
          <div>
            ...
            <p style="font-size:0.85rem;color:var(--text-secondary);margin:0;">
              ${isAr ? 'تسجيلات صوتية لمحاضرات دكاترة الكلية مرتبطة بالشيتات' : 'Faculty lecture audio recordings linked to lecture sheets'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="sheets-control-bar">
        <div class="sheets-dropdown-wrap">
          ...
          <select id="recordings-subject-dropdown" class="sheets-dropdown">
            <option value="all">${isAr ? 'كل المواد' : 'All Subjects'}</option>
            ${subjects.map(s => `<option value="${s.id}" ${currentFilter === s.id ? 'selected' : ''}>${isAr ? s.name_ar : s.name_en}</option>`).join('')}
          </select>
        </div>

        <div class="sheets-search-wrap">
          <i data-lucide="search" class="sheets-search-icon"></i>
          <input id="recordings-search-input" type="text" class="sheets-search-input"
            placeholder="${isAr ? 'ابحث عن شيت أو مادة...' : 'Search sheets or subjects...'}"
            autocomplete="off" />
          <button id="recordings-search-clear" class="sheets-search-clear" style="display:none;">
            <i data-lucide="x" style="width:14px;height:14px;"></i>
          </button>
        </div>

        <div class="sheets-control-right">
          <span id="recordings-count-label" class="sheets-count-label"></span>
          <div class="sheets-view-toggle">
            <span class="sheets-view-label">${isAr ? 'العرض' : 'View'}</span>
            <button class="sheets-view-btn ${currentView === 'grid' ? 'active' : ''}" data-view="grid" title="${isAr ? 'شبكة' : 'Grid'}">
              <i data-lucide="layout-grid" style="width:15px;height:15px;"></i>
            </button>
            <button class="sheets-view-btn ${currentView === 'list' ? 'active' : ''}" data-view="list" title="${isAr ? 'قائمة' : 'List'}">
      ...
      <div id="recordings-list" class="${currentView === 'grid' ? 'sheets-grid-container' : 'sheets-list-container-view'}"></div>
    `;

    renderList();

    // Subject dropdown
    document.getElementById('recordings-subject-dropdown')?.addEventListener('change', e => {
    ...
    const searchClear = document.getElementById('recordings-search-clear');
    let debounce;
    ...
    // View toggle
    container.querySelectorAll('.sheets-view-btn[data-view]').forEach(btn => {
      btn.addEventListener('click', () => {
        currentView = btn.getAttribute('data-view');
        localStorage.setItem('kf_recordings_view', currentView);
        container.querySelectorAll('.sheets-view-btn[data-view]').forEach(b => {
